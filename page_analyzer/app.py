...
from .data_base import URL_DB, get_connection
from .url import get_info
import os
import validators
import requests
...
def add_url():
    ...
    db = URL_DB()
    data_urls = db.get_data_by_name(conn, url)
    if data_urls:
        id = data_urls['id']
        flash('Страница уже существует', category='info')
        return redirect(url_for('url_page', id=id))
    db.save_to_db(conn, url)
    id = db.get_id_by_name(conn, url)
...
def url_page(id):
    conn = get_connection(DATABASE_URL)
    db = URL_DB()
    url_data = db.get_data_by_id(conn, id)
...
def check_url(id):
    ...
    db.save_check_to_db(conn, id, **url_info)
